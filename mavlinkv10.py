...
class MAVLink_pi_packet_message(MAVLink_message):
        '''
        The phase offset of a signal received by an SDR
        '''
        id = MAVLINK_MSG_ID_PI_PACKET
        name = 'PI_PACKET'
        fieldnames = ['magnitude', 'angle', 'antenna_type']
        ordered_fieldnames = [ 'magnitude', 'angle', 'antenna_type' ]
        format = '<ffB'
        ...
        def __init__(self, magnitude, angle, antenna_type):
                MAVLink_message.__init__(self, MAVLink_pi_packet_message.id, MAVLink_pi_packet_message.name)
                self._fieldnames = MAVLink_pi_packet_message.fieldnames
                self.magnitude = magnitude
                self.angle = angle
                self.antenna_type = antenna_type

        def pack(self, mav):
                return MAVLink_message.pack(self, mav, 48, struct.pack('<ffB', self.magnitude, self.angle, self.antenna_type))
...
mavlink_map = {
        ...
        MAVLINK_MSG_ID_PI_PACKET : MAVLink_pi_packet_message,
...
class MAVLink(object):
        ...
        def pi_packet_encode(self, magnitude, angle, antenna_type):
                '''
                The phase offset of a signal received by an SDR

                magnitude                 : Magnitude (float)
                angle                     : Angle (float)
                antenna_type              : Antenna Type (uint8_t)

                '''
                return MAVLink_pi_packet_message(magnitude, angle, antenna_type)

        def pi_packet_send(self, magnitude, angle, antenna_type):
                '''
                The phase offset of a signal received by an SDR
                ...
                '''
                return self.send(self.pi_packet_encode(magnitude, angle, antenna_type))
